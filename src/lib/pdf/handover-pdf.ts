...
import type { DocDamage, HandoverDocumentData } from "@/lib/handover-view";
...
const EXISTING_TITLE = "Bestehende Schäden (vor dieser Übergabe bereits dokumentiert)";
const NEW_TITLE = "Bei Übergabe dokumentierte Vorschäden";

function damageTable(pdf: Pdf, title: string, status: string, list: DocDamage[], emptyText: string) {
  ...
  pdf.table(
    ...
    list.map((d): Cell[] => [String(d.index), { text: status, bold: true, color: d.marker === "NEW" ? COLORS.bad : COLORS.ink2 }, d.viewLabel, d.kindLabel, d.description, d.severityLabel, d.size || "–"]),
    { zebra: true },
  );
}
...
function checklist(pdf: Pdf, items: HandoverDocumentData["checklist"]) {
  ...
  for (const item of items) {
    const result = (item.result || "nicht beantwortet").toUpperCase();
...
function sketch(pdf: Pdf, data: HandoverDocumentData, svg: string | null) {
  ...
  pdf.ensureSpace(44);
  let y = pdf.y + 2;
  drawMarker(pdf, "EXISTING", null, pdf.left + 6, y + 5, 4.2);
  y += pdf.textAt("Kreis: bestehender Schaden, bereits vor dieser Übergabe dokumentiert", pdf.left + 16, y, pdf.width - 16, { size: 8, color: COLORS.ink2 }) + 3;
  drawMarker(pdf, "NEW", null, pdf.left + 6, y + 5, 4.2);
  y += pdf.textAt(data.type === "PICKUP" ? "Raute: bei dieser Übergabe dokumentierter Vorschaden" : "Raute: bei dieser Rückgabe festgestellt", pdf.left + 16, y, pdf.width - 16, { size: 8, color: COLORS.ink2 }) + 4;
...
export async function renderHandoverPdf(data: HandoverDocumentData, assets: HandoverPdfAssets): Promise<{ bytes: Buffer; trace: PdfTrace }> {
  ...
  pdf.documentTitle(title, [
    ...
    data.finalizedAt ? `Übergabe abgeschlossen am ${data.finalizedAt}` : `Begonnen am ${data.startedAt}`,
  ]);
  ...
  pdf.sectionTitle("Übergabedaten");
  pdf.keyValues([
    { label: "Übergabe am", value: data.finalizedAt ?? data.startedAt },
  ...
  damageTable(pdf, EXISTING_TITLE, "BESTEHENDER SCHADEN", existing, "Vor dieser Übergabe waren keine Schäden dokumentiert.");
  damageTable(pdf, NEW_TITLE, data.type === "PICKUP" ? "BEI ÜBERGABE DOKUMENTIERTER VORSCHADEN" : "BEI RÜCKGABE FESTGESTELLT", fresh, "Bei dieser Übergabe wurden keine weiteren Schäden festgestellt.");
  if (data.type === "PICKUP" && fresh.length > 0) {
    pdf.paragraph("Diese Schäden waren bei der Übergabe bereits am Fahrzeug vorhanden. Sie wurden vor Fahrtantritt gemeinsam dokumentiert und gelten als Vorschäden.", { size: 8.5, color: COLORS.ink2 });
  }
