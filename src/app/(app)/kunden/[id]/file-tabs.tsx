        ...
        {f.documents.length === 0 && f.drafts.length === 0 ? <Empty>Noch keine Belege.</Empty> : (
          <div className="overflow-x-auto">
            <table className="w-full text-[13.5px]">
              ...
              <tbody>
                {f.drafts.map((d) => <tr key={d.id} className="border-b border-line-soft"><td className="px-3 py-2.5"><Link href={d.href} className="hover:underline text-ink-3">Entwurf</Link></td><td className="px-3 py-2.5">{INVOICE_DOCUMENT_TYPES[d.documentType as InvoiceDocumentTypeKey]}{d.kind === "DAMAGE" ? " · Schaden" : ""}</td><td className="px-3 py-2.5 text-ink-3" colSpan={5}>Buchung {d.bookingNumber} · noch nicht abgeschlossen</td><td className="px-3 py-2.5"><Chip tone="amber">Entwurf</Chip></td></tr>)}
                {f.documents.map((d) => {
                  ...
                  return (
                    <tr key={d.id} className={`border-b border-line-soft last:border-0 hover:bg-panel-2/60 ${counter ? "text-ink-2" : ""}`}>
                      ...
                      <td className="px-3 py-2.5">{INVOICE_DOCUMENT_TYPES[d.documentType as InvoiceDocumentTypeKey] ?? d.documentType}{d.kind === "DAMAGE" ? <span className="block text-[11px] text-ink-3">Schadenabrechnung</span> : null}{counter && d.original && <span className="block text-[11px] text-ink-3">zu {d.original.number}</span>}</td>
