...
export function NumberRangesForm({ prefixes, next }: { prefixes: Record<NumberRangeKey, string>; next: Record<NumberRangeKey, string> }) {
  ...
  return (
    <form action={formAction} className="grid grid-cols-1 md:grid-cols-3 gap-x-4 gap-y-3.5 p-5">
      ...
      <div className="md:col-span-3"><button disabled={pending} className="btn btn-primary">{pending ? "Wird gespeichert…" : "Nummernkreise speichern"}</button></div>
    </form>
  );
}
